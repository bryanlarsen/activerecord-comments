...
describe ActiveRecord::Comments, 'Oracle Enhanced' do
  ...
  def drop_table
    connection.execute 'DROP TABLE foxes;'
  end
  ...
  describe ActiveRecord::Base, 'extensions' do
    ...
    it "should create table OK" do
      connection.execute "CREATE TABLE foxes( id INTEGER NOT NULL, CONSTRAINT foxes_pk PRIMARY KEY (id));"
    ...
    it "Model#comment should return the database comment for a model that has a database comment" do
      connection.execute "CREATE TABLE foxes( id INTEGER NOT NULL, CONSTRAINT foxes_pk PRIMARY KEY (id));"
      connection.execute "COMMENT ON TABLE foxes IS 'foxes Rule';"
  ...
  describe ActiveRecord::ConnectionAdapters::Column, 'extensions' do
    ...
    it 'Model#column_comment should return the comment for a column' do
      connection.execute "CREATE TABLE foxes( id INTEGER NOT NULL, CONSTRAINT foxes_pk PRIMARY KEY (id));"
      connection.execute "COMMENT ON COLUMN foxes.id IS 'i am the ID column';"
